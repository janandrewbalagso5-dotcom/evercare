            ...
            {activeView === "report-panel" && reportPatient && (
                ...
                >
                    <AccordionSection
                        ...
                        open={openSections.medicalHistory}
                        onToggle={() => toggleSection("medicalHistory")}
                    >
                        <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 8 }}>
                            <tbody>
                                <InfoRow label="Blood Type" value={aptForPatient?.medicalHistory?.bloodType} shade />
                                <InfoRow label="Allergies" value={aptForPatient?.medicalHistory?.allergies} />
                                <InfoRow label="Current Medications" value={aptForPatient?.medicalHistory?.currentMedications} shade />
                                <InfoRow label="Past Conditions" value={aptForPatient?.medicalHistory?.pastConditions} />
                                <InfoRow label="Previous Surgeries" value={aptForPatient?.medicalHistory?.previousSurgeries} shade />
                            </tbody>
                        </table>
                        {aptForPatient?.medicalHistory?.files?.length > 0 && (
                            <div style={{ marginTop: 12 }}>
                                <div style={{ fontWeight: 700, fontSize: "0.83rem", marginBottom: 8, color: "#1c2e3e" }}>
                                    Uploaded Documents
                                </div>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                                    {aptForPatient.medicalHistory.files.map((f, i) => (
                                        <a
                                            key={i}
                                            href={f.dataUrl}
                                            download={f.name}
