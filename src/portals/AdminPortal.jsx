...
const apiFetch = async (path, options = {}) => {
    ...
    if (!res.ok) { const err = await res.json().catch(() => ({ error: res.statusText })); throw new Error(err.error || "API request failed"); }
    return res.json();
};
...
export default function AdminPortal({ currentUser, showNotification, onLogout }) {
    ...
    const handleAddDoctor = async (e) => {
        ...
        try {
            ...
            const email = newDoc.name.toLowerCase().replace(/\s/g, "") + "@evercare.com";

            // Save to users table
            await dbService.createUser({
                uid: docId, email, name: newDoc.name,
                role: "doctor", specialty: newDoc.specialty, phone: "+63 917 000 0000",
            });
            ...
            await apiFetch("/api/doctors", {
                method: "POST",
                body: JSON.stringify({
                    id: docId, name: newDoc.name, specialty: newDoc.specialty,
                    rating: 5.0, experience: 5, fee: Number(newDoc.fee),
                    avatar: newDoc.avatar, bio: newDoc.bio,
    ...
    const handleEditDoctor = async (e) => {
        ...
        try {
            await apiFetch(`/api/doctors/${editDoc.id}`, {
                method: "PATCH",
                body: JSON.stringify({
                    name: editDoc.name, specialty: editDoc.specialty,
                    fee: Number(editDoc.fee), bio: editDoc.bio, avatar: editDoc.avatar,
                }),
            });
    ...
    const handleDeleteDoctor = async (docId) => {
        ...
        try {
            await apiFetch(`/api/doctors/${docId}`, { method: "DELETE" });
            ...
            <div className="adm-body">
                ...
                <aside className={`adm-sidebar ${sidebarOpen ? "open" : ""}`}>
                    ...
                    <NavGroup icon={<Database size={16} />} label="Services" isOpen={openNavGroup === "services"} onToggle={() => toggleNavGroup("services")}>
                        ...
                        <NavItem label="Add Treatment" active={activeView === "add-treatment"} onClick={() => nav("add-treatment")} />
